...
kiss_obj* kiss_eql(kiss_obj* obj1, kiss_obj* obj2) {
    if (obj1->type == KISS_INTEGER && obj2->type == KISS_INTEGER) {
    ...
    if (obj1->type == KISS_CHARACTER && obj2->type == KISS_CHARACTER) {
...
static kiss_tagbody_t* kiss_make_tagbody(kiss_symbol_t* tag, jmp_buf jmp,
					 kiss_obj* body)
{
    kiss_environment_t* env = Kiss_Get_Environment();
...
static void kiss_eval_cleanups(kiss_obj* stop, kiss_obj* jmps) {
    kiss_environment_t* env = Kiss_Get_Environment();
...
static kiss_block_t* kiss_block_ref(kiss_symbol_t* name) {
	...
	if (KISS_IS_BLOCK(x)) {
	    ...
	    if (block->name == name) {
		if (kiss_member((kiss_obj*)block, env->lexical_env.jumpers) !=
		    KISS_NIL)
		{
		    return block;
		} else {
