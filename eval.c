...
static kiss_obj* kiss_invoke(kiss_obj* f, kiss_obj* args) {
    /* fwprintf(stderr, "invoke entered\n"); fflush(stderr); */
    ...
    case KISS_OBJECT: {
	if (kiss_cfuncall("generic-function-p", kiss_clist(1, f)) == KISS_T) {
	    /* fwprintf(stderr, "calling generic-function\n"); fflush(stderr); */
	    return kiss_cfuncall("generic-function-invoke",
    ...
    default:
	Kiss_Err("Can't invoke function like object ~S", f);
    }
}
...
static kiss_obj* kiss_eval_compound_form(kiss_cons_t* p) {
    ...
    default: Kiss_Err("Invalid compound expression ~S", p);
    }
}
...
kiss_obj* kiss_eval(kiss_obj* form) {
    ...
    kiss_obj* result;
    /****************************************************
    if (++env->call_nest > KISS_MAX_CALL_NEST) {
	Kiss_Err("Call nest is too deep ~S", form);
    }
    */
    /* wprintf("call_nest = %ld\n", env->call_nest); fflush(stdout); */
    switch (form->type) {
    case KISS_CONS: {
...
static kiss_obj* kiss_eval_args(kiss_obj* args) {
    ...
    for (args = Kiss_Proper_List(args); KISS_IS_CONS(args);
	 args = KISS_CDR(args))
    {
